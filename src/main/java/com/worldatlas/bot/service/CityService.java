...
public class CityService {
    ...
    public String getCityInfoWithWeather(City city, String lang, String timeFormat, com.worldatlas.bot.entity.User user) {
        ...
        // Получаем данные о погоде (нужны для восхода/заката даже если погода не показывается)
        Map<String, Object> weather = null;
        if (showWeather || showSunriseSunset) {
            weather = weatherService.getWeather(city.getName());
        }
        ...
        // Показываем восход/закат независимо от настройки погоды
        if (showSunriseSunset && weather != null) {
